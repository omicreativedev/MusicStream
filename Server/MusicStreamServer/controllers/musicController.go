...
import (
	...
	"github.com/omicreativedev/MusicStream/Server/MusicStreamServer/database"
	"github.com/omicreativedev/MusicStream/Server/MusicStreamServer/models"
	"github.com/omicreativedev/MusicStream/Server/MusicStreamServer/utils"
...
func GetMusic(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var musicCollection *mongo.Collection = database.OpenCollection("music", client)

		cursor, err := musicCollection.Find(ctx, bson.D{})
...
func GetMusic(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if musicID == "" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Music ID is required"})
			return
		}
		...
		err := musicCollection.FindOne(ctx, bson.D{{Key: "imdb_id", Value: musicID}}).Decode(&music)
...
func AdminReviewUpdate(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		musicId := c.Param("music_id")
		if musicId == "" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Music Id required"})
			return
		}
		var req struct {
		...
		filter := bson.D{{Key: "music_id", Value: musicId}}
...
func GetRecommendedMusics(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error fetching recommended music"})
			return
		}
		defer cursor.Close(ctx)
